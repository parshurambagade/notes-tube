...
if (!notes || !notes.content || notes.content.length === 0) {
  return <div className="min-h-screen bg-gray-900 text-gray-100 p-8 flex justify-center items-center">No notes found.</div>;
 } else {
    return (
      <div className="min-h-screen bg-gray-900 text-gray-100 p-8  w-full ">
        <div className="flex max-w-4xl justify-between gap-0 mb-3 mx-auto">
          ...
          <div className="flex gap-4 items-center">
            <button className="bg-violet-600 hover:bg-violet-700 text-white px-4 py-2 rounded-md transition-colors flex items-center gap-2" onClick={() => navigate(`/notes/edit/${notesId}`)}>
              ...
              <button className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md transition-colors flex items-center gap-2" onClick={handleDeleteClick}>
        ...
        <DeleteConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={handleConfirmDelete} noteTitle={notes.title} />
      </div>
    );
  }
};
