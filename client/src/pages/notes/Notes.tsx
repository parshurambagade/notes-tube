...
const NotesPage: React.FC<NotesPageTypes> = ({setAllowEditing, allowEditing, isSaved}) => {
  ...
  return (
    <div className="parent lg:px-0  pt-2 pb-8 w-full max-w-4xl border-t-gray-800 h-max bg-gray-900">
      ...
      {/* header of notes */}
      <NotesHeader isSaved={isSaved} title={notesData?.title} setAllowEditing={setAllowEditing} allowEditing={allowEditing} />
      {/* VIDEO  */}
      <VideoContainer videoId={notesVideo}/>

      {/* NOTES CONTENT  */}
      <NotesContent allowEditing={allowEditing && allowEditing } content={notesData.content} setContent={() => {}}/>
    </div>
  );
};
